...
if(interactive()) {
  ...
  ui <- dashboardPage(skin = "blue",
    ...
    .skin-blue .main-sidebar {
      background-color: #DAD000;
                        ...
                        verbatimTextOutput(outputId = "new_Coef"),
  ...
  server <- function(input, output, session) {
    ...
    output$Coef <- renderPrint(coef(lm1()))
    
    output$new_Coef <- renderPrint({
      RegData %>% split(.$gender) %>%
        map(~lm(reformulate(input$DependentVariable, input$IndependentVariable), data = .x))
    })
